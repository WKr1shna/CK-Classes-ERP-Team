...
class ResourcesFragment : Fragment() {
    ...
    private val repository = ResourceRepository(RetrofitClient.apiService)
    ...
    private fun loadResources() {
        ...
        lifecycleScope.launch {
            when (val res = repository.getResources()) {
                ...
                is NetworkResult.Error -> {
                    binding.swipeRefresh.isRefreshing = false
                    Toast.makeText(requireContext(), res.message ?: "Failed to load resources", Toast.LENGTH_SHORT).show()
                }

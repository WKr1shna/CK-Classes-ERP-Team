...
const notificationSchema = z.object({
  ...
  notifyOnRoomChange: z.boolean(),
  notifyOnSubstitution: z.boolean(),
...
export const NotificationsSection = ({ initialData, sectionId, searchQuery }) => {
  ...
  const mutation = useMutation({
    mutationFn: (data) => timetableSettingsService.updateSection('notifications', data),
    onSuccess: (data) => {
  ...
  return (
    <div id={sectionId} className={isMatch ? 'ring-2 ring-indigo-500 ring-offset-2 rounded-xl transition-all' : ''}>
      <SectionCard
        ...
        description="Configure automated messaging rules for timetable events."
        icon={Bell}
        isEditing={isEditing}
        ...
        <form className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              ...
              <label className="flex items-center space-x-3 cursor-pointer">
                <input type="checkbox" {...register('notifyOnChange')} className="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                <span className="text-sm font-medium text-slate-700">Timetable Changes (Draft Published)</span>
              </label>
              <label className="flex items-center space-x-3 cursor-pointer">
                <input type="checkbox" {...register('notifyOnRoomChange')} className="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                <span className="text-sm font-medium text-slate-700">Room Change Alerts</span>
              </label>
              <label className="flex items-center space-x-3 cursor-pointer">
                <input type="checkbox" {...register('notifyOnSubstitution')} className="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                <span className="text-sm font-medium text-slate-700">Teacher Substitution Alerts</span>
              </label>
            </div>
